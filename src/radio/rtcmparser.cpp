...
// ---------------------------------------------------------------------------
const uint32_t RTCMParser::CRC_LOOKUP[256] = {
...
RTCMParser::RTCMParser(std::initializer_list<uint8_t> preambles, PacketCallback callback)
  : callback_(std::move(callback)), idle_since_(ros::Time::now())
{
  preamble_count_ = 0;
  for (uint8_t p : preambles)
  {
    if (preamble_count_ < MAX_PREAMBLES)
    {
      preambles_[preamble_count_++] = p;
    }
    else
    {
      ROS_WARN("[RTCMParser] too many preambles, ignoring 0x%02X (max %zu)", p, MAX_PREAMBLES);
    }
  }
}
...
// feed()
// ---------------------------------------------------------------------------
void RTCMParser::feed(const uint8_t* data, size_t length)
{
...
void RTCMParser::feed(uint8_t byte)
{
  process_byte(byte);
}
...
// is_preamble()
// ---------------------------------------------------------------------------
bool RTCMParser::is_preamble(uint8_t byte) const
{
  for (size_t i = 0; i < preamble_count_; ++i)
  {
    if (preambles_[i] == byte) return true;
  }
  return false;
}

// ---------------------------------------------------------------------------
// process_byte() — state machine
// ---------------------------------------------------------------------------
void RTCMParser::process_byte(uint8_t byte)
{
  switch (state_)
  {
    ...
    case State::PAYLOAD:
      ...
      // Extract message type from the first 12 payload bits
      if (payload_count_ == 0) {
        msg_type_ = static_cast<uint16_t>(byte) << 4;
      }
      else if (payload_count_ == 1) {
        msg_type_ |= (byte >> 4) & 0x0Fu;
      }
...
bool RTCMParser::is_idle_at_least(uint32_t ms) const {
  ...
  if (idle_since_ == ros::Time::ZERO) return false;
